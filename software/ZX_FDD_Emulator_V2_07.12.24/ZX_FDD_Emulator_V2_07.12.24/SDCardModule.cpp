...
void setFullSpeed() // После инициализации запускаем карту на полную скорость
{
...
uint8_t send_cmd(uint8_t cmd, uint32_t param, uint8_t cnt)
{
  ...
  //  _delay_us(100);
   ...
   // for( i = 0; i < 128; i++) if((res = spiRead()) != 0xFF) break;  // wait for response and get it
    i = 200;								/* Wait for a valid response in timeout of 10 attempts */
	  do
		  res = spiRead();
	  while ((res & 0x80) && --i);
...
CSTATUS card_initialize (void)
{
  ...
  { // Entered Idle state
    ...
    { // SDv1 or MMC
      ...
      for (tmr = 25000; tmr && (re=send_cmd(cmd, 0, 0)); tmr--) ;  /* Wait for leaving idle state */
      if (!tmr || (re=send_cmd(CMD16, 512, 0)) != 0)  CardType = 0; // Set R/W block length to 512  
    }
    setFullSpeed();
  }
...
)
{
  ...
  { // READ_SINGLE_BLOCK
    ...
    { // receive data block + CRC
      p = (uint8_t*)dest;
      for(uint8_t i=0 ;; i++) { SPDR = 0xFF; *p++=spiRead_(); if(i==0xFF) break;}
      for(uint8_t i=0 ;; i++) { SPDR = 0xFF; *p++=spiRead_(); if(i==0xFF) break;}
      SPDR = 0xFF;  spiRead_();
      SPDR = 0xFF;  spiRead_();
    }
  ...
  if (buff) {   // Send data bytes
    ...
    if (sa) { // Initiate sector write process 
      ...
      if (( spiRead() & 0x1F) == 0x05) { // Receive data resp and wait for end of write process in timeout of 500ms
        for (bc = 5000; spiRead() != 0xFF && bc; bc--)  _delay_us(100); // Wait ready
        if (bc) res = RES_OK;
      }
...
uint32_t card_read_serial()
{
  ...
  { // READ_CID
      for(uint8_t i = 0; i < 16; i++) { SPDR = 0xFF;  *ptr++=spiRead_(); }
