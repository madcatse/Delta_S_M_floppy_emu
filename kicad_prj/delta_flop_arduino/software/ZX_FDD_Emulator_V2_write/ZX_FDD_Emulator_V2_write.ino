...
изображения на 180 градусов (Rahost). 
...
uint8_t sector_data[768]; // sector data
...
/*  
    UBRR0H = 0x00;
    UBRR0L =0x0F ; //0x07; //0x0F; // 1000 kbps for 16MHz external oscillator
    //UBRR0 = F_CPU/(1000000*2) - 1  ;
    UCSR0A = 0x00;
...
void spi_init()
{
    ...
    SPCR = _BV(MSTR) | _BV(SPE) | _BV(SPR0);   // Master mode, SPI enable, clock rate f_osc/16, LSB first
...
/*       
        oled.clearDisplay();
        oled.setTextXY(3,1);
        oled.putString(F("Card mount OK!"));
        _delay_ms(1000);
*/             
      #if (OLED == 1)
        oled.clearDisplay();
                ...
                if(f_index > 1){
                    ...
                    /*
                        ...
                        */
                        ...
                        oled.putChar(0);
                        #else
                        //LCD_print_char(0,disp_index + START_DISP_POS,0); 
                        ...
                        oled.putChar(62);
                        #else
                        //LCD_print_char(0,disp_index + START_DISP_POS,32);
                    ...
                    /*
                        ...
                        if(res == 0)  print_files(disp_index);
...
LCD_print(0,1, F("CYL: 00  HEAD: 0") );
        ...
        for(uint16_t i = fat.csize; i < max_cylinder*16; i+=fat.csize)
...
*/                      
            ...
            //           PORTB|= _BV(6) ;
                        ...
                        int8_t ret=  read_sector_(temp_ptr) ;
                        TCCR1B = 0  ;  // stop timer 
                        ...
                        for(uint8_t cnt = 0; ;){
                           temp = *temp_ptr++ ;
                           send_byte(temp );
                           CRC_H.val = (CRC_H.bytes.low << 8) ^ pgm_read_word_near(Crc16Table + (CRC_H.bytes.high ^ temp));
                           if(cnt++ == 255) break;
                        }
...
int8_t read_sector_(uint8_t *buf){
    int16_t size = 256 ;
    uint8_t ret ;
